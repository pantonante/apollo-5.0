...
std::unordered_map<int, int> ContiArsPreprocessor::local2global_map_;
...
void ContiArsPreprocessor::ExpandIds(drivers::ContiRadar* corrected_obstacles) {
  for (int iobj = 0; iobj < corrected_obstacles->contiobs_size(); ++iobj) {
    ...
    int corrected_id = 0;
    auto ob = local2global_map_.find(id);
    if (ob != local2global_map_.end()) {
      ...
      corrected_id = GetNextId();
      local2global_map_.insert({id, corrected_id});
    }
